...
type (
	...
	ConsistentHash struct {
		...
		keys     []uint64
		ring     map[uint64][]any
		nodes    map[string]langx.PlaceholderType
		lock     sync.RWMutex
	}
)
...
func NewCustomConsistentHash(replicas int, fn Func) *ConsistentHash {
	...
	return &ConsistentHash{
		...
		nodes:    make(map[string]langx.PlaceholderType),
	}
}
...
func (h *ConsistentHash) addNode(nodeRepr string) {
	h.nodes[nodeRepr] = langx.Placeholder
}
